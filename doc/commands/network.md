...
## LRS Maintenance, Network Diagnostics, Service Areas & Reverse Geocoding (placeholder commands)

> ⚠️ **Estado:** los cinco comandos de esta sección son cascarones (placeholders). Analizan y validan sus argumentos igual que el resto de Spatial TEC, pero no calculan todavía ningún resultado real: terminan imprimiendo un aviso de "no implementado" y no generan un archivo de salida. Se agregaron a partir de la revisión del manual `practicas/`, que ya asumía que existían. `spatial_lrs_info`, `spatial_network_info` y `spatial_network_service` agrupan bajo un solo binario operaciones que originalmente se pensaron como comandos separados, con la misma convención `-operation`/`-mode`/flag booleano que ya usan [`spatial_vectorize`](raster.md#spatial_vectorize) y [`spatial_merge_raster`](raster.md#spatial_merge_raster) en el resto del proyecto.

## spatial_lrs_info

**NAME**

`spatial_lrs_info` — información y validación de una red de referenciación lineal (LRS) *(placeholder)*

**SYNOPSIS**

```
spatial_lrs_info <network.lrs> [options]
```
...
| Option | Description |
|---|---|
| `-route <id>` | Mostrar el detalle de una sola ruta |
| `-validate` | Validar también la topología y las medidas (M-values): solapes, huecos, medidas fuera de rango |
| `-report <file>` | Guardar un reporte de los errores encontrados (solo relevante con `-validate`) |

**EXAMPLES**

```
spatial_lrs_info roads.lrs
spatial_lrs_info roads.lrs -route 101
spatial_lrs_info roads.lrs -validate -report lrs_errors.csv
```

**SEE ALSO**

[spatial_lrs_create](#spatial_lrs_create)
...
`spatial_network_info` — información y validación de conectividad de una red construida con `spatial_network` *(placeholder)*

**SYNOPSIS**

```
spatial_network_info <network.network> [options]
```
...
| Option | Description |
|---|---|
| `-validate` | Validar también la conectividad: nodos aislados, componentes desconectados |
| `-report <file>` | Guardar un reporte de los problemas encontrados (solo relevante con `-validate`) |
| `-verbose` | Mostrar información detallada |

**EXAMPLES**

```
spatial_network_info roads.network
spatial_network_info roads.network -validate -report issues.csv
```

**SEE ALSO**

[spatial_network](#spatial_network), [spatial_network_service](#spatial_network_service)
...
`spatial_network_service` — áreas de servicio (allocation) e isócronas desde un nodo origen *(placeholder)*

**SYNOPSIS**

```
spatial_network_service <network.network> -from <node> -mode <alloc|iso> [options]
```
...
| Mode | Description |
|---|---|
| `-mode alloc` | Una sola área de servicio bajo un `-max_cost` |
| `-mode iso` | Isócronas: un polígono por cada valor en `-intervals` |

**OPTIONS**

| Option | Description |
|---|---|
| `-from <node>` | Nodo origen (**requerido**) |
| `-mode <alloc|iso>` | Modo de servicio (**requerido**) |
| `-max_cost <value>` | Costo máximo acumulado (`alloc`; default: sin límite) |
| `-intervals <list>` | Intervalos de costo/tiempo separados por coma, ej. `5,10,15` (`iso`; **requerido** para ese modo) |
| `-output <file>` | Archivo CSV de salida (default: `service_result.csv`) |

**EXAMPLES**

```
spatial_network_service roads.network -from 0 -mode alloc -max_cost 10 -output service_area.csv
spatial_network_service roads.network -from 0 -mode iso -intervals 5,10,15 -output isochrones.csv
```

**SEE ALSO**

[spatial_network_info](#spatial_network_info), [spatial_shortest_path](#spatial_shortest_path)
...
`spatial_address_validate` — valida los rangos de direcciones de una capa de calles *(placeholder)*
...
| Option | Description |
|---|---|
| `-check_overlaps` | Detectar rangos de numeración solapados |
| `-check_gaps` | Detectar huecos en la numeración |
| `-report <file>` | Guardar un reporte de problemas encontrados |
...
[spatial_address](#spatial_address), [spatial_reverse_geocode](#spatial_reverse_geocode)
...
`spatial_reverse_geocode` — encuentra la dirección postal más cercana a uno o más puntos dados *(placeholder)*

**SYNOPSIS**

```
spatial_reverse_geocode <streets.csv> <addresses.csv> <points.csv> <output.csv> [options]
```
...
| Option | Description |
|---|---|
| `-max_dist <value>` | Distancia máxima de búsqueda (default: `50.0`) |

**EXAMPLES**

```
spatial_reverse_geocode streets.csv addresses.csv query_points.csv result.csv
```

**SEE ALSO**

[spatial_address](#spatial_address), [spatial_address_validate](#spatial_address_validate)
